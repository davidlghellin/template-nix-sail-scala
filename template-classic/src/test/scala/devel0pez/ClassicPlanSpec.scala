...
/** Plan measurements that only classic Spark can express.
  *
  * This spec lives in `template-classic` rather than beside the shared ones for a reason worth
  * knowing: Connect's `QueryExecution` is a different class, without `analyzed` or `optimizedPlan`.
  * Touching them is a **compile** error for the connect backend, not a runtime failure, so no
  * `perEngine` or `failsOnSail` branch can hold it — the code simply must not be compiled against
  * Connect. A backend-specific source directory is the only thing that says that.
  *
...
final class ClassicPlanSpec extends SparkSuite {
  ...
  /** Only meaningful against classic, so it says so instead of asking to be told. */
  override protected val backend: String = "classic"
